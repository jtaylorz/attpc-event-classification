"""
basicNN_sim2real_multic.py
==========================

Testing a basic neural network on nuclear scattering data.
Trains on simulated tests on real data.
"""
...
noise_sim_labels = np.full((noise_sim.shape[0],), 2)

#merging
full_sim_data = sp.sparse.vstack([p_sim, C_sim, noise_sim], format='csr')
full_sim_labels_categorical = np.hstack((p_sim_labels, C_sim_labels, noise_sim_labels))
full_sim_labels = np_utils.to_categorical(full_sim_labels_categorical)
...
model.add(Dense(full_sim_labels.shape[1], activation='softmax'))
...
textfile = open('../keras-results/NN/sim2real/multic.txt', 'w')
textfile.write('acc \n')
